...
class TiersDialog(QDialog):
    def __init__(self, parent, is_score=True, current_tiers=[]):
        ...
        self.setWindowTitle("Manage Score Tiers" if is_score else "Manage Penalty Tiers")
        layout = QVBoxLayout()
        self.table = QTableWidget()
        col_count = 3 if is_score else 4
        self.table.setColumnCount(col_count)
        headers = ["Min", "Label", "Color"] if is_score else ["Min", "Max", "Label", "Color"]
...
class ActionHandler:
    def __init__(self, main):
        ...
        self.main.set_score_col_action.triggered.connect(self.set_score_column)
        self.main.set_penalty_col_action.triggered.connect(self.set_penalty_column)
        self.main.set_formula_action.triggered.connect(self.set_score_formula)
        self.main.score_tiers_action.triggered.connect(self.set_score_tiers)
        self.main.penalty_tiers_action.triggered.connect(self.set_penalty_tiers)

    def set_score_tiers(self):
        dlg = TiersDialog(self.main, is_score=True, current_tiers=self.main.data_model.score_tiers)
        if dlg.exec_():
            tiers = dlg.get_tiers()
            self.main.data_model.score_tiers = tiers
    ...
    def set_penalty_tiers(self):
        ...
        if dlg.exec_():
            tiers = dlg.get_tiers()
            self.main.data_model.penalty_tiers = tiers
            self.main.data_model.save_to_history()
            self.main.update_legends()
            self.main.refresh_table()
            self.main.update_chart()
    ...
    def set_score_column(self):
        ...
        if not numeric_cols:
            ...
            return
        col, ok = QInputDialog.getItem(self.main, "Set Score Column", "Select column:", numeric_cols, 0, False)
        if ok:
            try:
                self.main.data_model.set_score_col(col)
                self.main.refresh_table()
                self.main.update_chart()
                self.main.update_legends()
                self.main.change_view(self.main.current_view)
    ...
    def set_penalty_column(self):
        numeric_cols = [col for col, typ in self.main.data_model.column_types.items() if typ in ["integer", "float", "boolean"] and col != self.main.data_model.score_col]
        if not numeric_cols:
            QMessageBox.warning(self.main, "No Columns", "No numeric columns available.")
            return
        col, ok = QInputDialog.getItem(self.main, "Set Penalty Column", "Select column:", numeric_cols, 0, False)
        if ok:
            try:
                self.main.data_model.set_penalty_col(col)
                self.main.refresh_table()
                self.main.update_chart()
                self.main.update_legends()
                self.main.change_view(self.main.current_view)
